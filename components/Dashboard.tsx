...
const Dashboard: React.FC<DashboardProps> = ({ user, loans, systemBudget, onViewAllLoans }) => {
  ...
  const displayedLoans = showAllHistory ? loans : loans.slice(0, 3);
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex flex-col items-end gap-1.5">
              ...
              <div className="flex items-center gap-1.5 bg-black/10 px-3 py-1.5 rounded-full border border-black/5">
                ...
                <span className="text-[10px] font-black uppercase">{user?.rankProgress || 0}/10 ĐIỂM</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className={`bg-[#111111] border rounded-[2.5rem] overflow-hidden transition-all ${isBudgetLow ? 'border-orange-500/30 ring-1 ring-orange-500/10' : 'border-white/5'}`}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1 text-right">
              <p className="text-[9px] font-bold text-gray-500 uppercase">Hợp đồng hoàn tất</p>
              <p className="text-xl font-black text-green-500">
                {loans.filter(l => l.status === 'ĐÃ TẤT TOÁN').length} HĐ
              </p>
            </div>
          </div>
        </div>
