...
const Login: React.FC<LoginProps> = ({ onLogin, onNavigateRegister, error }) => {
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Hỗ trợ nhập "ADMIN" hoặc "1111111111" cho tài khoản đặc biệt
    if (rawVal.toUpperCase() === 'ADMIN' || rawVal === '1111111111') {
      setPhone(rawVal.toUpperCase() === 'ADMIN' ? 'ADMIN' : rawVal);
      return;
    }
  ...
  const handleBlurPhone = () => {
    if (phone.length > 0 && phone.length < 10 && phone !== 'ADMIN' && phone !== '1111111111') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (phone === '') {
      ...
      setShowPhoneTooltip(true);
      hasError = true;
    } else if (phone !== 'ADMIN' && phone !== '1111111111' && phone.length < 10) {
      setPhoneErrorMsg('SỐ ZALO PHẢI ĐỦ 10 KÝ TỰ');
      setShowPhoneTooltip(true);
      hasError = true;
    }
